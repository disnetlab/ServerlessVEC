...
##                                 cls=DockerSta, ports=[80,8888], dcmd='./ConnectToCluster.sh', dimage="server_example:latest")
            ...
            car = net.addStation("c"+vehicleId,  mode='n',mac=randomMac, ip=ip_addr, cls=DockerSta, mem_limit='1024m', ports=[80,8888], dimage="server_example:latest", 
...
##                                position=pos,**kwargs)
        ...
        attached_vm = net.addHost("D"+apname, mac=randomMac, ip = "172.18.5.12/24",cls=Docker, ports=[80,8888], mem_limit='2048m', dcmd='fwatchdog', dimage="faisalyolo")
        dockerObj = net.get("D"+apname)
        print(dockerObj.cmd("curl -s --form \"image_file=@abc.jpg\"  http://172.18.5.12:8080/function/hello-python"))
        print(dockerObj.cmd("curl -s --form \"image_file=@abc.jpg\"  http://172.18.5.12:8080/function/hello-python"))
